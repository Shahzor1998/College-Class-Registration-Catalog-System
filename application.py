...
GMU, S2022 = DataSetUp()   # run the data load function

Menu = """

        1. Create Course
        2. Add Students to Course (Spring 2022)
    ...
    if menu_pic == '1':   # Create Course -----------------------------
        ...
        if item_pic != 'x':
            ...
            if faculty_pic != 'x':   # create the course and add to the Catalog
                ...
                if newCourse not in S2022:
                    S2022.addCourse(newCourse)
                    input('\n' + newCourse.getName() + ' added to ' + S2022.getName( ) +
                          '"Enter" to continue') 
                else:
                    print('Course already in catalog ', S2022.getName( ))
                
    if menu_pic == '2':   # Add students to course -----------------------
        course_pic = ' '
        while course_pic != 'x':  # allow for multiple students to be added at once
            c_list = S2022.listCourses( )  # Select course ------------------
